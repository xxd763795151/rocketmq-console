    ...
    $scope.openBelongItemDialog = function (request) {
        $http({
            ...
            if (resp.status == 0) {
                ...
                $.each(request.items, function (i, e){
                    choices.push(e.id);
                });
                ngDialog.open({
                    preCloseCallback: function (value) {
                    ...
                    controller: 'consumerBelongItemDialogController',
                    data: {
                        item: resp.data,
                        group: request.group,
                        choices: choices
...
module.controller('consumerBelongItemDialogController', ['$scope', 'ngDialog', '$http', 'Notification', function ($scope, ngDialog, $http, Notification) {
        $scope.postBelongItemRequest = function (requestItem) {
            requestItem.type = 2
            $http({
                method: "POST",
