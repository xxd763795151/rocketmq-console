...
public class MQAdminInstance {
    ...
    public static synchronized void initMQAdminInstance(long timeoutMillis) throws MQClientException {
        Integer nowCount = INIT_COUNTER.get();
        if (nowCount == null) {
            DefaultMQAdminExt defaultMQAdminExt;
            if (timeoutMillis > 0) {
                defaultMQAdminExt = AclClientHookHolder.HOLDER.isNotEmpty() ?
                        new DefaultMQAdminExt(AclClientHookHolder.HOLDER.getAclHook(), timeoutMillis)
                        : new DefaultMQAdminExt(timeoutMillis);
            } else {
                defaultMQAdminExt = AclClientHookHolder.HOLDER.isNotEmpty() ?
                        new DefaultMQAdminExt(AclClientHookHolder.HOLDER.getAclHook())
    ...
    public static synchronized void destroyMQAdminInstance() {
