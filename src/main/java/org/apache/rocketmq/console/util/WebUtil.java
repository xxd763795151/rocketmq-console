...
public class WebUtil {
    ...
    public static UserInfo getLoginInfo() {
        ...
        UserInfo userInfo = (UserInfo) getValueFromSession(request, USER_INFO);
        if (userInfo == null) {
            // Whether do not need to sign in
            RMQConfigure configure = BeanUtil.getBean(RMQConfigure.class);
            if (!configure.isLoginRequired()) {
                userInfo = new UserInfo();
                userInfo.setIp(getIp(request));
                userInfo.setUser(new User("admin", "", 1));
                return userInfo;
            }
        }
        return userInfo;
