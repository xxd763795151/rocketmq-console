    ...
    public void collectTopic() {
        ...
        try {
            ...
            for (String topic : topicSet) {
                if (topic.startsWith(MixAll.RETRY_GROUP_TOPIC_PREFIX) || topic.startsWith(MixAll.DLQ_GROUP_TOPIC_PREFIX) || TopicValidator.isSystemTopic(topic)) {
                    continue;
                }
                ...
                for (BrokerData bd : topicRouteData.getBrokerDatas()) {
                    ...
                    if (masterAddr != null) {
                        try {
                            ...
                            stopwatch.reset();
                            statsErrorLog("Exception caught: mqAdminExt get broker stats data TOPIC_PUT_NUMS failed", e);
                        }
                    }
                }
                        ...
                        for (BrokerData bd : topicRouteData.getBrokerDatas()) {
                            ...
                            if (masterAddr != null) {
                                try {
                                    ...
                                    statsErrorLog("Exception caught: mqAdminExt get broker stats data GROUP_GET_NUMS failed", e);
                                }
                            }
                        }
                    }
...
//    @Scheduled(cron = "0/5 * * * * ?")
    ...
    // Clean up file from 7 days ago
//    @Scheduled(cron = "0 0 1 * * ?")
    public void clearOutdatedFiles() {
        log.info("Start to clean up expired files.");
        String dataDirPath = rmqConfigure.getRocketMqConsoleDataPath();
        String prefix = "dashboard";
        File dataDir = new File(dataDirPath);
        if (!dataDir.exists() || !dataDir.isDirectory()) {
            log.warn("directory is invalid, path: {}", dataDirPath);
            return;
        }

        final String dateFormat = "yyyy-MM-dd";
        Arrays.stream(dataDir.listFiles()).filter(file -> file.getName().startsWith(prefix) && file.getName().endsWith(".json")).forEach(file -> {
            String name = file.getName().split("\\.")[0];
            String dateStr = name.substring(prefix.length(), prefix.length() + dateFormat.length());
            Date fileDate = null;
            try {
                fileDate = DateUtils.parseDate(dateStr, dateFormat);
            } catch (ParseException e) {
                log.error("Parse file date error, file name: " + file.getName(), e);
            }

            Date daysAgo = DateUtils.addDays(new Date(), -7);
            if (fileDate.compareTo(daysAgo) < 0) {
                if (file.delete()) {
                    log.info("Delete expired file: {}", file.getAbsolutePath());
                }
            }
        });
    }
    ...
    private void statsErrorLog(String des, Exception e) {
        LogUtil.statsErrorLog(log, des, e);
    }
}
